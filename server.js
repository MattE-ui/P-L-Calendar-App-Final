...
const T212_LEADER_ALERT_OFF_HOURS_INTERVAL_MS = (() => {
  const parsed = Number(process.env.T212_LEADER_ALERT_OFF_HOURS_INTERVAL_MS);
  if (!Number.isFinite(parsed) || parsed < 30000) return 120000;
  return Math.min(parsed, 15 * 60 * 1000);
})();
const T212_LEADER_ALERT_NO_POSITION_INTERVAL_MS = (() => {
  const parsed = Number(process.env.T212_LEADER_ALERT_NO_POSITION_INTERVAL_MS);
  if (!Number.isFinite(parsed) || parsed < 60000) return 5 * 60 * 1000;
...
function userLeadsTradeGroups(db, username) {
  return getTradeGroupsLedByUser(db, username).length > 0;
}
...
function buildTradeGroupClosureEventKey(trade, reason = 'snapshot_disappearance') {
  const normalizedReason = String(reason || 'snapshot_disappearance').trim().toLowerCase() || 'snapshot_disappearance';
  return `${String(trade?.id || '').trim() || 'unknown-trade'}:${normalizedReason}`;
}

function hasTradeGroupSellAlertBeenEmittedForTrade(trade, closureEventKey = '') {
  if (!trade || typeof trade !== 'object') return false;
  if (trade.tradeGroupSellAlertEmitted === true) return true;
  const emittedEventKey = String(trade.tradeGroupSellAlertEventKey || '').trim();
  return Boolean(closureEventKey && emittedEventKey && emittedEventKey === closureEventKey);
}

function markTradeGroupSellAlertEmittedForTrade(trade, closureEventKey = '') {
  if (!trade || typeof trade !== 'object') return;
  trade.tradeGroupSellAlertEmitted = true;
  trade.tradeGroupSellAlertEventKey = closureEventKey || String(trade.tradeGroupSellAlertEventKey || '').trim() || null;
  trade.tradeGroupSellAlertEmittedAt = new Date().toISOString();
}

function emitTradeGroupSellAlertForClosedTrade(db, leaderUsername, trade, {
  ...
  if (!groups.length) {
    console.info('[TradeGroup][SellAlert]', {
      ...
      tradeId: trade?.id || null,
      ticker: resolveCanonicalTickerForTrade(trade) || null,
      reason: 'no_groups_led',
      closureEventKey
    });
    markTradeGroupSellAlertEmittedForTrade(trade, closureEventKey);
  ...
  for (const group of groups) {
    ...
    const alert = {
      ...
      leader_user_id: leaderUsername,
      source_trade_id: trade.id,
      broker_source: trade?.source === 'trading212' || trade?.trading212Id ? 'trading212' : null,
      ticker,
      side: 'SELL',
  ...
  if (alertsCreated > 0 || duplicates > 0) {
    markTradeGroupSellAlertEmittedForTrade(trade, closureEventKey);
  }
  console.info('[TradeGroup][SellAlert]', {
...
function buildTrading212FillEventKey(fillEvent = {}) {
  ...
  if (fillId) return `fill:${fillId}`;
  const orderId = String(fillEvent?.orderId || '').trim();
  const side = String(fillEvent?.side || '').trim().toUpperCase();
  const ticker = String(fillEvent?.ticker || '').trim().toUpperCase();
  const filledAt = String(fillEvent?.filledAt || '').trim();
...
function emitTradeGroupAlertFromTrading212Fill(db, leaderUsername, fillEvent = {}) {
  ...
  const side = String(fillEvent?.side || '').toUpperCase();
  ...
  if (side === 'SELL' && sourceTrade && hasTradeGroupSellAlertBeenEmittedForTrade(sourceTrade)) {
    ...
    console.info('[TradeGroup][SellAlert]', {
      ...
      ticker: resolveCanonicalTickerForTrade(sourceTrade) || fillEvent?.ticker || null,
      reason: 'duplicate',
      skipReason: 'trade_close_alert_already_emitted'
    });
    return { alertsCreated: 0, notificationsCreated: 0, duplicates: 1 };
  }
  let alertsCreated = 0;
  ...
  for (const group of groups) {
    ...
    const alert = {
      ...
      group_id: group.id,
      leader_user_id: leaderUsername,
      source_trade_id: fillEvent?.sourceTradeId || null,
      ...
      filled_at: fillEvent?.filledAt || null,
      created_at: nowIso
    };
    db.tradeGroupAlerts.push(alert);
    alertsCreated += 1;
  ...
  if (side === 'SELL' && sourceTrade && (alertsCreated > 0 || duplicates > 0)) {
    markTradeGroupSellAlertEmittedForTrade(sourceTrade, buildTradeGroupClosureEventKey(sourceTrade, 'history_reconcile'));
  }
  return { alertsCreated, notificationsCreated, duplicates };
}
...
function computeEligibilityReason(settings, stats, mode = 'trade') {
  ...
  if (mode === 'trade' && stats.trade_count < 3) return 'insufficient_trades';
  return null;
}
...
function explainFormula(periodKey, path, diagnostics = {}) {
  ...
  if (path === 'account_source_equity_history') {
    return 'Using source-specific account equity history adjusted for net cash flows.';
  }
...
function buildAccountModeReturnFromEntries(entries, periodKey) {
  if (!Array.isArray(entries) || !entries.length) {
    return { returnPct: null, reason: 'missing_source_history_entries', details: null };
  }

  if (periodKey === 'ALL') {
    if (entries.length < 2) {
      return { returnPct: null, reason: 'insufficient_source_history_points', details: null };
    }
    const startEquity = Number(entries[0].end);
    const endEquity = Number(entries[entries.length - 1].end);
    if (!Number.isFinite(startEquity) || startEquity === 0 || !Number.isFinite(endEquity)) {
      return { returnPct: null, reason: 'invalid_source_history_equity', details: null };
    }
    const inPeriod = entries.slice(1);
    const deposits = inPeriod.reduce((sum, entry) => sum + Math.max(0, Number(entry.cashIn) || 0), 0);
    const withdrawals = inPeriod.reduce((sum, entry) => sum + Math.max(0, Number(entry.cashOut) || 0), 0);
  ...
  const periodEntries = entries.filter(entry => entry.date >= startKey);
  if (!priorEntries.length || !periodEntries.length) {
    return { returnPct: null, reason: 'insufficient_source_history_window', details: null };
  }
  const startEquity = Number(priorEntries[priorEntries.length - 1].end);
  const endEquity = Number(periodEntries[periodEntries.length - 1].end);
  if (!Number.isFinite(startEquity) || startEquity === 0 || !Number.isFinite(endEquity)) {
...
function buildLeaderboardStatDiagnostics(db, username, periodKey, mode = 'trade', options = {}) {
  const user = db.users?.[username];
  if (!user) return null;
  const includeTrades = options.includeTrades === true;
  const selectedMode = LEADERBOARD_MODE_VALUES.includes(mode) ? mode : 'trade';
  ...
  const sourceHistory = listChronologicalEntriesForSource(user, selectedSource);
  const accountResult = buildAccountModeReturnFromEntries(sourceHistory.entries, periodKey);

  let resolvedReturnPct = null;
  let calculationPath = 'return_unavailable';
  if (selectedMode === 'account') {
    resolvedReturnPct = Number.isFinite(accountResult.returnPct) ? accountResult.returnPct : null;
    calculationPath = Number.isFinite(accountResult.returnPct)
      ? 'account_source_equity_history'
      : 'return_unavailable';
  } else {
  ...
  const reason = computeEligibilityReason(settings, stat, selectedMode);
  ...
  if (selectedMode === 'account') {
    diagnostics.account_history_details = accountResult.details;
    diagnostics.unavailable_reason = accountResult.reason || sourceHistory.reason || null;
  } else if (calculationPath === 'history_based') {
...
function computeLeaderboardStatForUser(db, username, periodKey, mode = 'trade') {
  const built = buildLeaderboardStatDiagnostics(db, username, periodKey, mode);
  return built?.stat || null;
}

function upsertLeaderboardStat(db, stat) {
  ensureSocialTables(db);
  const index = db.leaderboardStats.findIndex(s => (
    s.user_id === stat.user_id
    && s.period_key === stat.period_key
    && (s.leaderboard_mode || 'trade') === (stat.leaderboard_mode || 'trade')
  ));
...
function recomputeLeaderboardStatsForUser(db, username) {
  ...
  for (const mode of LEADERBOARD_MODE_VALUES) {
    for (const period of LEADERBOARD_PERIOD_KEYS) {
      const stat = computeLeaderboardStatForUser(db, username, period, mode);
      if (stat) {
...
function ensureTradingAccounts(user) {
  ...
  existing.forEach((account, index) => {
    ...
    const id = rawId || crypto.randomBytes(6).toString('hex');
    ...
    normalized.push({
      id,
      label: normalizeTradingAccountLabel(account.label, `Account ${index + 1}`),
...
function ensureUserShape(user, identifier) {
  ...
  const { mutated: riskSettingsMutated } = ensureRiskSettings(user);
  if (riskSettingsMutated) mutated = true;
  return mutated;
}
...
function listChronologicalEntriesForSource(user, source) {
  const history = ensurePortfolioHistory(user);
  const sourceKey = source === 'trading212' || source === 'ibkr' ? source : null;
  if (!sourceKey) {
...
const TRADE_GROUP_SELL_DISAPPEARANCE_CONFIRMATION_SNAPSHOTS = Math.max(1, Number.parseInt(process.env.TRADE_GROUP_SELL_DISAPPEARANCE_CONFIRMATION_SNAPSHOTS || '2', 10) || 2);
...
function ensureTrading212LeaderAlertState(cfg, accountId = '__default__') {
  if (!cfg || typeof cfg !== 'object') return {};
  if (!cfg.leaderAlertState || typeof cfg.leaderAlertState !== 'object' || Array.isArray(cfg.leaderAlertState)) {
    cfg.leaderAlertState = { accounts: {} };
  }
  if (!cfg.leaderAlertState.accounts || typeof cfg.leaderAlertState.accounts !== 'object' || Array.isArray(cfg.leaderAlertState.accounts)) {
    cfg.leaderAlertState.accounts = {};
  }
  const key = accountId || '__default__';
  if (!cfg.leaderAlertState.accounts[key] || typeof cfg.leaderAlertState.accounts[key] !== 'object' || Array.isArray(cfg.leaderAlertState.accounts[key])) {
    cfg.leaderAlertState.accounts[key] = { positionByKey: {}, pendingChanges: [], disappearanceCandidates: {}, lastRunAt: '' };
  }
  const state = cfg.leaderAlertState.accounts[key];
  if (!state.positionByKey || typeof state.positionByKey !== 'object' || Array.isArray(state.positionByKey)) {
    state.positionByKey = {};
  }
  if (!Array.isArray(state.pendingChanges)) {
    state.pendingChanges = [];
  }
  if (!state.disappearanceCandidates || typeof state.disappearanceCandidates !== 'object' || Array.isArray(state.disappearanceCandidates)) {
    state.disappearanceCandidates = {};
  }
  return state;
}
...
function processLeaderTradeDisappearancesAfterValidSnapshot(db, {
  user,
  username,
  cfg,
  accountId = '',
  ...
  if (!user || !username || !cfg) return { mutated: false, emitted: 0 };
  const leaderState = ensureTrading212LeaderAlertState(cfg, accountId);
  const disappearanceCandidates = leaderState.disappearanceCandidates || {};
  const journal = ensureTradeJournal(user);
  const openTrades = [];
  for (const entries of Object.values(journal || {})) {
    for (const trade of (entries || [])) {
      ...
      if (trade.trading212AccountId && accountId && trade.trading212AccountId !== accountId) continue;
      if (!trade.trading212PositionKey) continue;
  ...
  for (const trade of openTrades) {
    const positionKey = String(trade.trading212PositionKey || '').trim();
    if (!positionKey) continue;
    const isPresent = Object.prototype.hasOwnProperty.call(presentPositionByKey, positionKey);
    if (isPresent) {
      if (disappearanceCandidates[trade.id]) {
        delete disappearanceCandidates[trade.id];
        mutated = true;
        console.info('[TradeGroup][SellAlert]', {
          stage: 'disappearance_cancelled_reappearance',
          leader: username,
    ...
    const current = disappearanceCandidates[trade.id] && typeof disappearanceCandidates[trade.id] === 'object'
      ? disappearanceCandidates[trade.id]
      : {
        tradeId: trade.id,
        ticker: resolveCanonicalTickerForTrade(trade) || null,
        positionKey,
        consecutiveValidAbsenceSnapshots: 0,
        firstDetectedAt: '',
        lastDetectedAt: ''
      };
    current.consecutiveValidAbsenceSnapshots = Number(current.consecutiveValidAbsenceSnapshots || 0) + 1;
    const nowIso = new Date().toISOString();
    current.firstDetectedAt = current.firstDetectedAt || nowIso;
    current.lastDetectedAt = nowIso;
    disappearanceCandidates[trade.id] = current;
    mutated = true;
    console.info('[TradeGroup][SellAlert]', {
      stage: 'ticker_disappearance_detected',
      leader: username,
      accountId: accountId || 'default',
      tradeId: trade.id,
    ...
    console.info('[TradeGroup][SellAlert]', {
      stage: 'disappearance_confirmation_progress',
      leader: username,
      accountId: accountId || 'default',
      tradeId: trade.id,
      ticker: current.ticker || null,
      progress: `${current.consecutiveValidAbsenceSnapshots}/${TRADE_GROUP_SELL_DISAPPEARANCE_CONFIRMATION_SNAPSHOTS}`
    });
    if (current.consecutiveValidAbsenceSnapshots >= TRADE_GROUP_SELL_DISAPPEARANCE_CONFIRMATION_SNAPSHOTS) {
      const emittedResult = emitTradeGroupSellAlertForClosedTrade(db, username, trade, {
        reason: 'snapshot_disappearance',
        classification: 'full_close'
      });
      if (emittedResult.alertsCreated > 0) emitted += emittedResult.alertsCreated;
      delete disappearanceCandidates[trade.id];
      mutated = true;
    }
  }
  leaderState.disappearanceCandidates = disappearanceCandidates;
  return { mutated, emitted };
}
...
function isUsMarketHours(now = new Date()) {
  ...
  return minuteOfDay >= (9 * 60 + 30) && minuteOfDay <= (16 * 60 + 10);
}
...
function extractTrading212HistoryOrders(payload) {
  ...
  if (Array.isArray(payload?.items)) return payload.items;
  if (Array.isArray(payload?.orders)) return payload.orders;
  if (Array.isArray(payload?.historyOrders)) return payload.historyOrders;
  if (Array.isArray(payload?.results)) return payload.results;
  if (Array.isArray(payload?.value)) return payload.value;
  if (Array.isArray(payload?.data)) return payload.data;
  return null;
...
function parseTrading212HistoryOrders(payload) {
  ...
  return orders.map(raw => {
    ...
    const normalizedQuantity = Number.isFinite(quantity) ? Math.abs(quantity) : null;
    return {
...
function ensureRiskSettings(user) {
  if (!user || typeof user !== 'object') {
    return { settings: { useSelectedTradingAccountsForRisk: false, selectedTradingAccountIdsForRisk: [] }, mutated: false };
  }
  let mutated = false;
  if (!user.riskSettings || typeof user.riskSettings !== 'object') {
    user.riskSettings = {};
    mutated = true;
  }
  if (typeof user.riskSettings.useSelectedTradingAccountsForRisk !== 'boolean') {
    user.riskSettings.useSelectedTradingAccountsForRisk = false;
    mutated = true;
  }
  const selectedIds = Array.isArray(user.riskSettings.selectedTradingAccountIdsForRisk)
    ? user.riskSettings.selectedTradingAccountIdsForRisk
      .map(id => String(id || '').trim())
      .filter(Boolean)
    : [];
  if (!Array.isArray(user.riskSettings.selectedTradingAccountIdsForRisk)
    || selectedIds.length !== user.riskSettings.selectedTradingAccountIdsForRisk.length) {
    user.riskSettings.selectedTradingAccountIdsForRisk = selectedIds;
    mutated = true;
  }
  return {
...
function getRiskCapitalBase(user, options = {}) {
  ...
  const defaultRiskCapital = Number(snapshot?.value);
  const fallbackCapital = Number.isFinite(defaultRiskCapital) && defaultRiskCapital >= 0 ? defaultRiskCapital : 0;
  ...
  const selectedRiskCapital = selectedAccounts.reduce((sum, account) => sum + (Number(account.usableValue) || 0), 0);
  let resolvedCapital = fallbackCapital;
  let appliedSelection = false;
  let fallbackReason = 'default_risk_capital_path';
  if (featureAvailable && settings?.useSelectedTradingAccountsForRisk === true) {
    if (selectedAccounts.length > 0) {
      resolvedCapital = selectedRiskCapital;
      appliedSelection = true;
      fallbackReason = 'selection_applied';
    } else {
      fallbackReason = 'selection_empty_after_filter_fallback';
    }
  } else if (!featureAvailable) {
    fallbackReason = 'feature_unavailable';
  } else if (settings?.useSelectedTradingAccountsForRisk !== true) {
    fallbackReason = 'selection_disabled';
  }
  const result = {
    riskCapitalBase: resolvedCapital,
    fallbackCapital,
    featureAvailable,
    eligibleAccounts,
    selectedAccounts,
    excludedAccounts,
    totalEligibleRiskCapital,
    selectedRiskCapital,
    selectionMode: settings?.useSelectedTradingAccountsForRisk ? 'selected_accounts' : 'default_portfolio',
    appliedSelection,
    fallbackReason,
...
function getPortfolioGBPForRisk(user) {
  ...
  return Number.isFinite(riskCapital.riskCapitalBase) && riskCapital.riskCapitalBase >= 0
    ? riskCapital.riskCapitalBase
    : 0;
}
...
function buildTrading212HistoryOrdersEndpoint({ pageLimit = 50, cursor = '' } = {}) {
  const safeLimit = Math.min(50, Math.max(1, Number(pageLimit) || 50));
  const params = new URLSearchParams();
  params.set('limit', String(safeLimit));
  if (cursor) params.set('cursor', String(cursor));
  return `/api/v0/equity/history/orders?${params.toString()}`;
}

function resolveTrading212HistoryNextPagePath(payload = {}) {
  if (!payload || typeof payload !== 'object') return '';
  const nextPagePath = payload.nextPagePath ?? '';
  if (!nextPagePath || typeof nextPagePath !== 'string') return '';
...
async function fetchTrading212HistoryOrders(config, username, {
  ...
  const headers = buildTrading212AuthHeaders(config);
  const safeLimit = Math.min(50, Math.max(1, Number(pageLimit) || 50));
  const endpointBase = '/api/v0/equity/history/orders';
  const cacheKey = `${String(username || '')}|${String(accountId || '')}|${baseUrl}|${safeLimit}|${String(ticker || '').trim().toUpperCase()}|${String(cursor || '')}`;
  const cached = trading212HistoryOrdersCache.get(cacheKey);
  if (!bypassCache && cached && Date.now() - cached.fetchedAt < TRADING212_HISTORY_ORDERS_CACHE_MS) {
    console.info(`[T212] cache hit history orders key=${cacheKey}`);
    return { ...cached.payload, fromCache: true };
  }
  console.info(`[T212] cache miss history orders key=${cacheKey}`);
  const rawItems = [];
  const pages = [];
  let nextPagePath = '';
  ...
  if (cursor) firstParams.set('cursor', String(cursor));
  let endpointPath = `${endpointBase}?${firstParams.toString()}`;
  let pageCount = 0;
  const debug = {
    historyOrdersRequestUrl: '',
    historyOrdersStatus: null,
    historyOrdersRawBody: '',
  ...
  do {
    const normalizedPath = endpointPath.startsWith('http')
      ? (() => {
          const parsed = new URL(endpointPath);
          return `${parsed.pathname}${parsed.search}`;
        })()
      : endpointPath;
    if (!normalizedPath.startsWith(endpointBase)) {
      const err = new Trading212ParseError(`Trading 212 history next page path must target ${endpointBase}.`, {
        status: 200,
        details: { endpointPath: normalizedPath }
      });
      err.debug = { ...debug, historyOrdersError: err.message };
      throw err;
    }
    ...
    const payload = upstream?.payload ?? null;
    const pageItems = extractTrading212HistoryOrders(payload);
    const nextRaw = resolveTrading212HistoryNextPagePath(payload);
    debug.historyOrdersRequestUrl = upstream?.url || url;
    debug.historyOrdersStatus = upstream?.status ?? null;
    debug.historyOrdersRawBody = upstream?.bodyText || '';
    ...
    if (payload === null || !Array.isArray(pageItems)) {
      const err = new Trading212ParseError('Trading 212 history orders response was in an unexpected format.', {
        ...
        details: { sample: String(upstream?.bodyText || '').slice(0, 500) }
      });
      debug.historyOrdersError = err.message;
      err.debug = { ...debug };
      throw err;
    }
  ...
  trading212HistoryOrdersCache.set(cacheKey, {
    fetchedAt: Date.now(),
    payload: response
  });
  return response;
...
function buildTrading212HistoryFillKey(fill = {}, accountId = '') {
  const orderId = fill.orderId || fill.order?.id || fill.id || '';
  const normalizedFillId = fill.fill?.id || fill.fillId || '';
  const normalizedSide = fill.order?.side || fill.side || '';
  const instrumentKey = fill.order?.instrumentUid
    || fill.instrumentUid
    || fill.order?.instrumentIsin
    || fill.instrumentIsin
    || fill.order?.ticker
    || fill.instrumentTicker
    || '';
  const quantity = fill.fill?.quantity ?? fill.quantity ?? '';
  const price = fill.fill?.price ?? fill.fillPrice ?? '';
  const filledAt = fill.fill?.filledAt ?? fill.filledAt ?? '';
  return [
    accountId || '',
    orderId,
    normalizedFillId,
...
function reconcileTrading212HistoricalExits(user, cfg, historyOrdersPayload, accountId = '', activeOrdersPayload = null, rates = {}) {
  ...
  for (const fill of fills) {
    ...
    for (const candidate of candidates) {
      ...
      const normalizedFillId = String(fill.fillId || fill.fill?.id || '');
      if (normalizedFillId) importedFillIds.add(normalizedFillId);
      ...
      nextExecutions.push({
        ...
        date: executionDate,
        brokerTradeId: fillOrderId || undefined,
      ...
      importedFillEvents.push({
        ...
        realizedPnlGbp: Number.isFinite(Number(trade.realizedPnlGBP)) ? Number(trade.realizedPnlGBP) : null,
...
async function fetchTrading212Positions(config, options = {}) {
  const baseUrl = resolveTrading212BaseUrl(config);
  const headers = buildTrading212AuthHeaders(config);
  const root = '/api/v0';
  const candidates = [
  ...
  for (const candidate of candidates) {
    try {
      ...
      if (Array.isArray(list)) {
        return { positions: list, debug: { endpoint: candidate, status: 200 } };
      }
    } catch (error) {
      if (error instanceof Trading212Error && error.status === 404) continue;
      throw error;
    }
  }
  throw new Trading212Error('Trading 212 positions endpoint not found.', { status: 404 });
}
...
const trading212LeaderJobs = new Map();
...
async function syncTrading212ForUser(username, runDate = new Date(), options = {}) {
  ...
  if (existingLock) {
    console.info(`[T212][${lane}] sync skipped username=${username} reason=lock_in_progress`);
    const db = loadDB();
    const user = db.users?.[username];
    if (user?.trading212 && typeof user.trading212 === 'object') {
  ...
  const syncPromise = (async () => {
    ...
    if (lane === 'leader_alert' && !userLeadsTradeGroups(db, username)) {
      ...
      return { skipped: true, reason: 'no_groups_led' };
    }
    ...
    for (const [index, account] of accounts.entries()) {
      ...
      const leaderAlertState = ensureTrading212LeaderAlertState(cfg, accountId);
      const isLeaderLane = lane === 'leader_alert';
      const shouldFetchSummary = !isLeaderLane;
      const shouldFetchHistory = true;
      const hasOpenTrading212Positions = Object.values(ensureTradeJournal(user)).flat().some(trade => (
        trade
        && trade.status !== 'closed'
      ...
      if (shouldFetchSummary && (!summaryGate.skip || snapshotStale)) {
        ...
        const positionsPayload = await fetchTrading212Positions(accountConfig, { accountId });
        snapshot = {
          portfolioValue: Number(user.portfolio) || 0,
          positions: positionsPayload.positions,
          transactions: [],
          raw: { positions: positionsPayload.positions, transactions: [] }
        };
        accountState.lastSnapshotAt = new Date().toISOString();
      }
      if (isLeaderLane) {
      ...
      } else if (isLeaderLane) {
        console.info(`[TradeGroup][LeaderSync] skipped endpoint=orders account=${accountId} reason=no_open_positions`);
      }
      ...
      if (shouldRunHistoryReconcile && !historyBlockedByCooldown) {
        ...
        try {
          historyOrdersPayload = await fetchTrading212HistoryOrders(accountConfig, username, {
            accountId,
            pageLimit: isLeaderLane ? 20 : 50,
            cursor: '',
            checkpoint,
            maxPages: isLeaderLane ? 1 : 3
          });
      ...
      accountResults.push({
        ...
        value: {
          ...
          leaderPositionsValid: isLeaderLane && Array.isArray(snapshot?.positions),
    ...
    if (lane === 'leader_alert' && userLeadsTradeGroups(db, username)) {
      for (const result of fulfilled) {
        if (!result.leaderPositionsValid) continue;
        processLeaderTradeDisappearancesAfterValidSnapshot(db, {
          user,
          username,
          cfg,
          accountId: result.accountId || '',
          presentPositionByKey: result.leaderPositionByKey || {}
        });
      }
    }
    ...
    const leaderPositionQuantityByAccount = {};
    if (positionEntries.length) {
      ...
      for (const entry of sortedPositions) {
        ...
        if (lane === 'leader_alert') {
          const positionKey = accountId ? `${accountId}:${symbol}` : symbol;
          leaderPositionQuantityByAccount[accountId] ||= {};
          leaderPositionQuantityByAccount[accountId][positionKey] = Math.abs(Number(quantity) || 0);
        }
    ...
    if (lane === 'leader_alert' && userLeadsTradeGroups(db, username)) {
      for (const result of fulfilled) {
        ...
        const leaderState = ensureTrading212LeaderAlertState(cfg, accountId);
        const prev = leaderState.positionByKey || {};
        const next = leaderPositionQuantityByAccount[accountId] || {};
        for (const [key, qty] of Object.entries(next)) {
          const previousQty = Number(prev[key] || 0);
          if (Math.abs(previousQty - qty) > 1e-8) {
            const existingPending = leaderState.pendingChanges.find(item => item.key === key);
            if (!existingPending) {
              leaderState.pendingChanges.push({
                id: crypto.randomUUID(),
                key,
                previousQty,
                nextQty: qty,
            ...
            console.info(`[TradeGroup][LeaderSync] position delta pending leader=${username} account=${accountId || 'default'} key=${key} from=${previousQty} to=${qty}`);
          }
        }
        leaderState.positionByKey = next;
        leaderState.lastRunAt = new Date().toISOString();
      }
    } else if (fulfilled.length) {
      const openTrading212Trades = openTrades.filter(entry => {
        const trade = entry?.trade;
        return trade && (trade.trading212Id || trade.source === 'trading212');
      });
      if (openTrading212Trades.length) {
        console.info(`[T212][sync] preserving_open_positions_on_snapshot_gap username=${username} openTrades=${openTrading212Trades.length} reason=positions_snapshot_empty_or_unavailable`);
      }
      for (const entry of openTrading212Trades) {
        const trade = entry.trade;
...
function stopTrading212Job(username) {
  ...
  const leaderJob = trading212LeaderJobs.get(username);
  if (leaderJob) {
    clearInterval(leaderJob.handle);
    trading212LeaderJobs.delete(username);
  }
}
...
function scheduleTrading212Job(username, user) {
  ...
  if (userLeadsTradeGroups(loadDB(), username)) {
    const leaderHandle = setInterval(async () => {
      const db = loadDB();
      const liveUser = db.users?.[username];
      if (!liveUser?.trading212?.enabled) {
        console.info(`[TradeGroup][LeaderSync] skipped leader=${username} reason=integration_disabled`);
        return;
      }
      if (!userLeadsTradeGroups(db, username)) {
        console.info(`[TradeGroup][LeaderSync] skipped leader=${username} reason=no_groups_led`);
        return;
      }
      const hasOpenPositions = Object.values(ensureTradeJournal(liveUser)).flat().some(trade => (
        trade
        && trade.status !== 'closed'
        && (trade.source === 'trading212' || trade.trading212Id)
      ));
      const cadence = hasOpenPositions
        ? (isUsMarketHours(new Date()) ? T212_LEADER_ALERT_MARKET_HOURS_INTERVAL_MS : T212_LEADER_ALERT_OFF_HOURS_INTERVAL_MS)
        : T212_LEADER_ALERT_NO_POSITION_INTERVAL_MS;
      const cfg = liveUser.trading212;
      const lastRunTs = Date.parse(cfg?.leaderAlertState?.lastRunAt || cfg?.lastLeaderAlertSyncAt || '');
      if (Number.isFinite(lastRunTs) && (Date.now() - lastRunTs) < cadence) {
        console.info(`[TradeGroup][LeaderSync] skipped leader=${username} reason=cadence`);
        return;
      }
      const result = await syncTrading212ForUser(username, new Date(), { lane: 'leader_alert' });
      if (!result?.skipped) {
        const dbAfter = loadDB();
        const latestUser = dbAfter.users?.[username];
        if (latestUser?.trading212) {
          latestUser.trading212.lastLeaderAlertSyncAt = new Date().toISOString();
          if (!latestUser.trading212.leaderAlertState || typeof latestUser.trading212.leaderAlertState !== 'object') {
            latestUser.trading212.leaderAlertState = {};
          }
          latestUser.trading212.leaderAlertState.lastRunAt = latestUser.trading212.lastLeaderAlertSyncAt;
          saveDB(dbAfter);
        }
      }
    }, T212_LEADER_ALERT_SYNC_TICK_MS);
    trading212LeaderJobs.set(username, { type: 'interval', handle: leaderHandle });
  }
}
...
app.post('/api/portfolio', auth, (req,res)=>{
  ...
  const resolvedAccountId = resolveTradingAccountId(user, req.body?.accountId);
  if (resolvedAccountId) {
    const currentAccount = accounts[resolvedAccountId] && typeof accounts[resolvedAccountId] === 'object'
    ...
    history[ym][dateKey].accounts = {
      ...
      [resolvedAccountId]: {
        ...
        cashIn: Number(currentAccount.cashIn) || 0,
        cashOut: Number(currentAccount.cashOut) || 0
      }
    };
  } else {
    history[ym][dateKey].accounts = {};
...
app.get('/api/profile', auth, asyncHandler(async (req,res)=>{
  ...
  res.json({
    ...
    riskSettings: {
      useSelectedTradingAccountsForRisk: !!riskSettings.useSelectedTradingAccountsForRisk,
...
async function fetchWatchlistTickerMetrics(db, ticker) {
...
app.post('/api/social/trade-groups/:groupId/members', auth, (req, res) => {
  ...
  const existingMember = db.tradeGroupMembers.find(item => item.group_id === group.id && item.user_id === friendUserId && item.status === 'active');
  if (existingMember) return res.status(409).json({ error: 'User is already a group member.' });
  const pendingInvite = db.tradeGroupInvites.find(item => item.group_id === group.id && item.invitee_user_id === friendUserId && item.status === 'pending');
  if (pendingInvite) return res.status(409).json({ error: 'Invite already pending for this user.' });

  const nowIso = new Date().toISOString();
  const invite = {
    id: crypto.randomUUID(),
    group_id: group.id,
    inviter_user_id: req.username,
    invitee_user_id: friendUserId,
    status: 'pending',
    created_at: nowIso,
    responded_at: null
  };
  ...
  logSocialEvent(db, {
    userId: friendUserId,
    actorUserId: req.username,
    eventType: 'social.trade_group_invite_sent',
    entityType: 'trade_group_invite',
    entityId: invite.id,
...
app.post('/api/social/trade-groups/invites/:inviteId/accept', auth, (req, res) => {
  if (rejectGuest(req, res)) return;
  const db = loadDB();
  if (!requireSocialNickname(db, req.username, res)) return;
  ensureSocialTables(db);
  const invite = db.tradeGroupInvites.find(item => item.id === req.params.inviteId);
  if (!invite) return res.status(404).json({ error: 'Invite not found.' });
  if (invite.invitee_user_id !== req.username) return res.status(403).json({ error: 'Only invitee can accept this invite.' });
  ...
  const existing = db.tradeGroupMembers.find(item => item.group_id === group.id && item.user_id === req.username);
  if (existing) {
    existing.status = 'active';
    existing.role = existing.role === 'leader' ? 'leader' : 'member';
...
app.post('/api/social/trade-groups/invites/:inviteId/decline', auth, (req, res) => {
  if (rejectGuest(req, res)) return;
  const db = loadDB();
  if (!requireSocialNickname(db, req.username, res)) return;
  ensureSocialTables(db);
  const invite = db.tradeGroupInvites.find(item => item.id === req.params.inviteId);
  if (!invite) return res.status(404).json({ error: 'Invite not found.' });
  if (invite.invitee_user_id !== req.username) return res.status(403).json({ error: 'Only invitee can decline this invite.' });
  if (invite.status !== 'pending') return res.status(400).json({ error: 'Invite is no longer pending.' });
...
app.post('/api/social/trade-groups/:groupId/invites/:inviteId/cancel', auth, (req, res) => {
  if (rejectGuest(req, res)) return;
  const db = loadDB();
  if (!requireSocialNickname(db, req.username, res)) return;
  ensureSocialTables(db);
  const group = db.tradeGroups.find(item => item.id === req.params.groupId && item.is_active !== false);
  if (!group) return res.status(404).json({ error: 'Trade group not found.' });
  if (group.leader_user_id !== req.username) return res.status(403).json({ error: 'Only leader can cancel invites.' });
  const invite = db.tradeGroupInvites.find(item => item.id === req.params.inviteId && item.group_id === group.id);
  if (!invite) return res.status(404).json({ error: 'Invite not found.' });
  if (invite.status !== 'pending') return res.status(400).json({ error: 'Only pending invites can be cancelled.' });
...
app.delete('/api/social/trade-groups/:groupId', auth, (req, res) => {
  if (rejectGuest(req, res)) return;
  const db = loadDB();
  if (!requireSocialNickname(db, req.username, res)) return;
  ensureSocialTables(db);
...
app.delete('/api/social/trade-groups/:groupId/alerts/:alertId', auth, (req, res) => {
  if (rejectGuest(req, res)) return;
  const db = loadDB();
  if (!requireSocialNickname(db, req.username, res)) return;
  ensureSocialTables(db);
  const group = db.tradeGroups.find(item => item.id === req.params.groupId && item.is_active !== false);
  if (!group) return res.status(404).json({ error: 'Trade group not found.' });
  if (group.leader_user_id !== req.username) return res.status(403).json({ error: 'Only leader can delete alerts.' });
  const before = db.tradeGroupAlerts.length;
  db.tradeGroupAlerts = db.tradeGroupAlerts.filter(item => !(item.group_id === group.id && item.id === req.params.alertId));
  if (db.tradeGroupAlerts.length === before) return res.status(404).json({ error: 'Alert not found.' });
  saveDB(db);
  res.json({ ok: true });
});
...
app.get('/api/social/trade-groups/:groupId/eligible-friends', auth, (req, res) => {
  ...
  db.tradeGroupInvites
    .filter(invite => invite.group_id === group.id && invite.status === 'pending')
    .forEach(invite => existing.add(invite.invitee_user_id));
...
app.get('/api/devtools/leaderboard-diagnostics', auth, (req, res) => {
  ...
  for (const mode of LEADERBOARD_MODE_VALUES) {
    timeframe[mode] = {};
    for (const period of LEADERBOARD_PERIOD_KEYS) {
      const built = buildLeaderboardStatDiagnostics(db, req.username, period, mode, { includeTrades: true });
      if (!built) continue;
      timeframe[mode][period] = built.diagnostics;
    }
  }
...
app.get('/api/social/leaderboard', auth, (req, res) => {
  ...
  const modeRaw = typeof req.query?.mode === 'string' ? req.query.mode.toLowerCase() : 'trade';
  const selectedMode = LEADERBOARD_MODE_VALUES.includes(modeRaw) ? modeRaw : 'trade';
  ...
  const entries = db.leaderboardStats
    .filter(s => (s.leaderboard_mode || 'trade') === selectedMode)
    ...
    .map(s => {
      ...
      return {
        ...
        leaderboard_mode: s.leaderboard_mode || 'trade',
  ...
  saveDB(db);
  res.json({ period: selectedPeriod, verification, mode: selectedMode, entries });
});
...
app.post('/api/profile', auth, (req,res)=>{
  ...
  if (hasMultiAccounts) {
    ...
    accountRecords[normalizedAccounts[0].id] = {
      end: portfolioNumber,
      cashIn,
      cashOut
    };
...
app.post('/api/account/risk-settings', auth, (req, res) => {
  if (rejectGuest(req, res)) return;
  const db = loadDB();
  const user = db.users[req.username];
  if (!user) return res.status(404).json({ error: 'User not found' });
  ...
  if (Array.isArray(req.body?.selectedTradingAccountIdsForRisk)) {
    settings.selectedTradingAccountIdsForRisk = req.body.selectedTradingAccountIdsForRisk
      .map(id => String(id || '').trim())
      .filter(Boolean);
  }
  const riskCapital = getRiskCapitalBase(user, {
...
app.get('/api/integrations/trading212/raw', auth, async (req, res) => {
  ...
  const accounts = getTrading212Accounts(cfg);
  if (accounts.length) {
    console.info(`[T212][raw] user=${req.username} configuredMode=${cfg.mode || 'live'} accountCount=${accounts.length}`);
    const results = [];
    for (const [index, account] of accounts.entries()) {
      const accountId = account.id || `account-${index + 1}`;
      const accountConfig = {
    ...
    const mergedAccounts = accounts.map((account, index) => {
      ...
      return {
        ...
        label: account.label || '',
        mode: account.mode || cfg.mode || 'live',
        resolvedBaseUrl: resolveTrading212BaseUrl({
          ...cfg,
          mode: account.mode || cfg.mode,
          baseUrl: account.baseUrl || cfg.baseUrl
        }),
    ...
    return res.json({
      ...
      historyOrdersError: primary.historyOrdersError ?? null,
...
function isWeekdayMarketDate(dateKey) {
...
function ensureWatchlistPreviousCloseReferences(db) {
  if (!db.watchlistPreviousCloseReferences || typeof db.watchlistPreviousCloseReferences !== 'object' || Array.isArray(db.watchlistPreviousCloseReferences)) {
    db.watchlistPreviousCloseReferences = {};
    return true;
  }
  return false;
}

function upsertWatchlistPreviousCloseReference(db, {
  ticker,
  marketDate,
  previousClose,
  source = 'unknown',
  ...
  if (!ticker || !marketDate) return false;
  const parsed = Number(previousClose);
  if (!Number.isFinite(parsed) || parsed <= 0) return false;
  ensureWatchlistPreviousCloseReferences(db);
  const key = `${String(ticker).toUpperCase()}|${marketDate}`;
  const existing = db.watchlistPreviousCloseReferences[key];
  const next = {
  ...
  const changed = !existing
    || Number(existing.previousClose) !== next.previousClose
    || String(existing.source || '') !== next.source
    || String(existing.currency || '') !== String(next.currency || '')
    || String(existing.exchange || '') !== String(next.exchange || '');
  if (changed) {
    db.watchlistPreviousCloseReferences[key] = next;
  }
  return changed;
}

function resolveWatchlistPreviousCloseReference(db, ticker, marketDate) {
  ensureWatchlistPreviousCloseReferences(db);
  const normalizedTicker = String(ticker || '').trim().toUpperCase();
  if (!normalizedTicker || !marketDate) return null;
  const exact = db.watchlistPreviousCloseReferences[`${normalizedTicker}|${marketDate}`];
  if (exact && Number.isFinite(Number(exact.previousClose)) && Number(exact.previousClose) > 0) {
    return exact;
  }
  const prefix = `${normalizedTicker}|`;
  const candidateKeys = Object.keys(db.watchlistPreviousCloseReferences)
    .filter((key) => key.startsWith(prefix))
    .map((key) => db.watchlistPreviousCloseReferences[key])
...
app.post('/api/trades/import/ibkr', auth, express.raw({ type: '*/*', limit: '10mb' }), async (req, res) => {
  ...
  try {
    ...
    for (let index = 0; index < table.rows.length; index += 1) {
      ...
      const normalizedShareQuantity = assetClassRaw === 'OPT' ? quantityAbs * multiplier : quantityAbs;
      const sizeUnits = normalizedShareQuantity;
      ...
      const normalizedTrade = normalizeTradeMeta({
        ...
        ibkrRawContractQuantity: assetClassRaw === 'OPT' ? quantityAbs : undefined,
        ibkrNormalizedShareQuantity: normalizedShareQuantity,
      ...
      parsedRows.push({
        ...
        brokerTradeId,
        fingerprint,
        executionQuantity: normalizedShareQuantity,
        buySell,
        openCloseIndicator,
        assetClassRaw,
        row
