...
async function loadConfig() {
  ...
  fields.connectorPath.value = config.connectorPath || '';
...
async function saveConfig() {
  const config = {
    ...
    connectorPath: fields.connectorPath.value.trim(),
...
document.getElementById('browse-connector').addEventListener('click', async () => {
  const folder = await window.veracityInstaller.browseFolder();
  if (folder) fields.connectorPath.value = folder;
});
