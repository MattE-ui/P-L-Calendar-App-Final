...
const CONFIG_DIR = path.join(process.env.APPDATA || path.join(os.homedir(), 'AppData', 'Roaming'), 'Veracity');
const CONFIG_PATH = path.join(CONFIG_DIR, 'ibkr.json');
...
function createTray() {
  ...
  const menu = Menu.buildFromTemplate([
    ...
    { label: 'Open Gateway Login', click: () => shell.openExternal('https://localhost:5000') },
    { label: 'Start Sync', click: () => startConnector() },
    { label: 'Stop Sync', click: () => stopConnector() },
    { type: 'separator' },
...
function resolveConnectorPath(config) {
  ...
  const exePath = path.join(__dirname, 'veracity-ibkr-connector.exe');
  if (fs.existsSync(exePath)) return exePath;
  return path.join(__dirname, 'app', 'connector', 'index.js');
}

function resolveGatewayRunbat(config) {
  if (!config.gatewayPath) return null;
  const runBat = path.join(config.gatewayPath, 'bin', 'run.bat');
  return fs.existsSync(runBat) ? runBat : null;
}

function startGateway() {
  const config = readConfig();
  const runBat = resolveGatewayRunbat(config);
  if (!runBat) {
    return { ok: false, message: 'Gateway path not set or run.bat missing.' };
  }
  if (gatewayProcess && !gatewayProcess.killed) {
    return { ok: true };
  }
  gatewayProcess = spawn(runBat, ['root\\conf.yaml'], {
    cwd: path.dirname(runBat),
    windowsHide: true
  });
...
ipcMain.handle('open-gateway-ui', () => shell.openExternal('https://localhost:5000'));
