...
function computeTradeRealizedPnlGBP(trade) {
  ...
  if (totalEntered > 0 && totalExited > 0) {
    ...
    if (Number.isFinite(netPnl)) {
      return convertPnlToGBP(netPnl, trade);
    }
  }
  ...
  if (Number.isFinite(entryP) && entryP > 0 && Number.isFinite(qty) && qty > 0 && Number.isFinite(closeP) && closeP >= 0) {
    ...
    return convertPnlToGBP(netPnlNative, trade);
  }
  ...
  sqliteDb.transaction(() => {
    ...
    for (const [username, user] of Object.entries(users)) {
      ...
      for (const trade of trades) {
        ...
        // computeTradeRealizedPnlGBP is authoritative — it already checks trade.ppl
        // first internally, so its return value is always preferable to re-checking
        // stored fields that may carry stale values from a previous bad sync.
        const computedPnlGBP = computeTradeRealizedPnlGBP(trade);
        const resolvedPnlGBP = computedPnlGBP ?? null;
