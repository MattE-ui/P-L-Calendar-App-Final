...
function computeTradeRealizedPnlGBP(trade) {
  ...
  if (totalEntered > 0 && totalExited > 0) {
    ...
    if (Number.isFinite(netPnl)) return convertPnlToGBP(netPnl, trade);
  }
  ...
  if (Number.isFinite(entryP) && entryP > 0 && Number.isFinite(qty) && qty > 0 && Number.isFinite(closeP) && closeP >= 0) {
    ...
    const grossPnlNative = (closeP - entryP) * qty * dir;
...
// For GBP trades: identity. For non-GBP: uses positionGBP/positionCurrency ratio
// stored on the trade (set by T212 / IBKR imports at write time).
