...
function formatIbkrOptionExpiry(yyMMdd) {
...
function parseIbkrOptionSymbol(symbolRaw) {
  const symbol = String(symbolRaw || '');
  if (!symbol.trim()) return null;
  const compact = symbol.replace(/\s+/g, ' ').trim();
  const match = compact.match(/^([A-Z0-9._-]+)\s+(\d{6})([CP])(\d{8})$/i);
  if (!match) return null;
  const [, underlyingRaw, expiryRaw, cpRaw, strikeRaw] = match;
  const expirationDate = formatIbkrOptionExpiry(expiryRaw);
  const strike = Number(strikeRaw) / 1000;
  if (!expirationDate || !Number.isFinite(strike) || strike <= 0) return null;
  return {
    underlyingTicker: String(underlyingRaw || '').toUpperCase(),
    expirationDate,
    optionType: cpRaw.toUpperCase() === 'P' ? 'put' : 'call',
    strike
  };
}
